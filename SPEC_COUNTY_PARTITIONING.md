...
## 10. Rulings & Amendments — August 15, 2026 (v1.2, from the real post-migration incident)

Same pattern as §9's own v1.1 amendment: the sections above (§1-§9) are left unedited. This section records what actually happened after the migration §5/§9.13 designed and tested was run against production, and what changed as a result. Unlike §9, this amendment is not a pre-implementation review — it is the post-incident record §9.6 anticipated ("this is the lifecycle framework applying to its own infrastructure") and the DALLAS-GATE-1 brief's own required output.

### 10.1 The incident this amendment exists to record

After the real migration ran (per §5/§9.13, `migrate_county_partitioning.py`), `county_code` became the new **leading** column of the composite primary key on every table §1's inventory named — exactly the design §4.3/§9.2(a) called for, and exactly the change `resolve_parcel()`'s docstring (§9.5) predicted the "218-call-site blast radius" for. That prediction proved accurate: application code across `app.py` and `loaders/` continued issuing `geo_id`-only (or `prop_id`-only, or `entity_code`-only) `WHERE` clauses against these tables — the pattern that was safe under the old, single-column PK, and remained *functionally* correct after the migration (Travis is still the only county with data, so an unscoped query still returns the right rows) but silently lost its index range-scan path, since Postgres cannot use a leading-column composite index/PK from a query that only filters on a non-leading column. Real production symptom: a class of query that used to be an index lookup became a sequential scan the moment its table's PK gained a leading column — the same failure mode named abstractly in §9.5, now the reason a dedicated audit tool (`verify_index_coverage.py`, `POST-PARTITION-INCIDENT-1-AUDIT`) was built and run against the real codebase, and the reason this v1.2 amendment exists.

### 10.2 The verification triad — formalized, not previously named as a set

The incident and its remediation this session (audit tool build, coverage-gap fixes, and DALLAS-GATE-1's routing/resolver work) collectively exercised three distinct kinds of correctness that this spec's earlier sections addressed separately but never named as one required triad for any future county-partitioning-shaped change:

1. **Mechanism safety** — does the migration/DDL itself execute correctly, atomically, without corrupting data or requiring a maintenance window. Covered by §5's migration plan and proven by §9.13's real test.
2. **Data correctness** — do the post-migration rows contain the right values (right `county_code`, right row counts, right sums) reconciled against source of truth. Covered by §5 step 2/§9.2(c)'s reconciliation requirement and `migrate_county_partitioning.py`'s own real reconciliation checks (`test_migrate_county_partitioning.py`).
3. **Workload compatibility** — do the queries the application actually issues against the new schema still have an efficient index path. **This is the gap §9 did not cover and the incident exposed:** §9.2(a)'s "county_code leads every composite key and index — no exceptions" rule was a correct *schema* rule, but nothing in v1.1 required verifying that every real, existing call site's `WHERE`/`JOIN` shape was updated to match it. Mechanism safety and data correctness were both proven (§9.13); workload compatibility was not checked at all until this incident forced it.

**Ruled:** any future migration that changes a table's leading key column(s) — county-partitioning-shaped or otherwise — must pass all three checks before being considered complete, not just the first two. `verify_index_coverage.py` (built this session, `test_verify_index_coverage.py`'s 24 fixture tests covering it) is the concrete tool for check 3; checks 1 and 2 already had concrete tools (§5's swap script, its own reconciliation logic) before this incident.

### 10.3 Amended index rule — transitional-index lifecycle

§9.2(a)'s rule ("county_code leads every composite key and index — no exceptions") stands, unchanged, as the *steady-state* target. This amendment adds the transitional step the incident showed was missing: the rule as originally written describes the end state but says nothing about the gap between "the PK's leading column changed" and "every real call site's WHERE clause was updated to filter on it."

**Amended rule:** at the moment a table's PK gains a new leading column (`county_code`), any **secondary** index that lets an existing, not-yet-updated call site keep an efficient access path on its *old* leading column (`geo_id`, `prop_id`, `entity_code`, as originally keyed) is a **required transitional artifact**, not an optional performance nicety — added in the same migration, not deferred. This is exactly the shape of the two real gaps closed in §10.5 below (`tax_billing(tax_year)`, `tax_billing_quarantine(geo_id)`) and the pattern this file's own prior reactive-index comments (the two waves preceding this one) already followed without it being written down as a formal rule.

**Transitional-index lifecycle, made explicit:**
1. **At migration time:** add the transitional secondary index alongside the new composite PK, for every column shape a real, currently-existing call site depends on (found via `verify_index_coverage.py`'s extraction, not guessed).
2. **During the county_code-wiring pass** (the DALLAS-GATE-1 Part 2 class of work — updating call sites to filter by `county_code` explicitly): each call site that gets updated to include `county_code` in its `WHERE`/`JOIN` reduces its dependency on the transitional index and becomes able to use the composite PK/leading-column index instead.
3. **Retirement condition:** a transitional index may be dropped once `verify_index_coverage.py --index-source live`'s own coverage report shows zero real call sites still relying on it (i.e., every call site that used to need the old-column-only path has been updated to filter by `county_code`). Never dropped on a schedule or a guess — only on that tool's own confirmation.
4. **Until retirement, transitional indexes are load-bearing,** not technical debt to be quietly cleaned up — removing one before its retirement condition is met silently reintroduces this exact incident for whatever call sites still depend on it.

### 10.4 The Dallas hard gate — formal written policy, not a schema.sql comment

`schema.sql`'s existing index comments (the two prior reactive-index waves) recorded *what* was fixed and *why*, informally, at the point of each fix — useful history, but not a forward-looking gate that has to be checked before the *next* county's data loads. This amendment makes that gate formal, in the one document DALLAS-GATE-1's own onboarding work is required to reference.

**Ruled — the Dallas hard gate:** before any real Dallas parcel data is loaded into this schema (i.e., before `DATA_LIFECYCLE.md`'s Dallas onboarding prerequisites — Source Registry, County Profile, Classification Map — are acted on), `verify_index_coverage.py --index-source live` must be run against the production database and must report:

- **Zero CONFIRMED gaps** (a real, currently-reachable query with no usable index path at all), and
- **Zero UNCONFIRMED findings** (a query that only appears covered via a stale inline `schema.sql` PK comment rather than a real, live secondary index or the live composite PK itself — see `verify_index_coverage.py`'s own stale-PK-vs-real-index distinction, built this session specifically because `schema.sql`'s inline PK text had drifted from the real post-migration PK for 14 tables).

This is the same underlying condition §9.2(a)'s rule always implied — "every county-keyed query has a real index path" — now stated as a literal, checkable pass/fail gate with a named command, rather than left as an implication of a schema-design rule. A CONFIRMED or UNCONFIRMED finding at gate-check time means a second county's rows would hit the exact same silent-sequential-scan failure this incident already caused for Travis-only data, at genuinely larger (multi-county) scale.

**This gate is not yet passed as of this amendment.** DALLAS-GATE-1's own work this session (Part 2's county_code-wiring pass, Part 3a's two schema.sql index fixes) made real, verified progress toward it but explicitly did not complete full 218+-call-site coverage in one pass — see this brief's own final report to Diego for the itemized remaining work. The gate stays open, and Dallas data loading stays blocked behind it, until a live re-run reports the zero/zero result above.

### 10.5 This incident's real remediation — what was actually done

Recorded here as the formal reason for this amendment, per the brief's own instruction:

1. **`verify_index_coverage.py` built** — a general-purpose, AST/regex-based SQL-query-vs-database-index coverage auditor, covering `app.py`, `loaders/`, and `tax_logic/`. Extends beyond `geo_id` to `prop_id`/`entity_code` composite-key risk (the same leading-column risk shape, other columns). Distinguishes real secondary-index coverage from untrustworthy matches against `schema.sql`'s stale inline PK text (§10.4). 24 fixture tests (`test_verify_index_coverage.py`), including a direct reproduction of this incident's pre-fix/post-fix behavior.
2. **A critical bug in the tool itself, found and fixed before any report was finalized** — a regex word-boundary defect (`_PREDICATE_RE`) silently failed to match the single most common predicate shape in this codebase (`column = %s` followed by whitespace), undercounting real coverage by roughly 20x in one before/after comparison during this session's own tool development. Caught via direct interactive testing against the real extraction output, not assumed correct — the kind of self-verification this spec's own §9.12/§9.13 verification discipline calls for, applied to a new tool built in response to this incident.
3. **Two real, confirmed index gaps closed in `schema.sql`** (§10.3's transitional-index pattern): `idx_billing_year` on `tax_billing(tax_year)` (`load_tax_current.py`'s `new_only` mode, and `backfill_tax_billing_2025_confidence.py`'s three UPDATE passes, all filter by `tax_year` alone) and `idx_quarantine_geo` on `tax_billing_quarantine(geo_id)` (`quarantine_contamination.py`'s `restore()` path, filters by `geo_id` alone).
4. **County-in-URL routing built** (DALLAS-GATE-1 Part 1/2, resolving §9.5's deferred routing decision and §9.5's routing/SEO alignment note): county slug is a URL path segment (`/travis-tx/...`, matching the slug format already live in `templates/index.html`'s `MARKETS` array — confirmed against real repo evidence, not the brief's own looser `/travis-county/...` example, per Diego's explicit decision this session), propagated per-request via Flask's `url_value_preprocessor`/`url_defaults` idiom (`app.py`'s `_pull_county_slug`/`_add_county_slug`), with all 27 pre-existing routes gaining a permanent 301 redirect from their old, un-prefixed paths.
5. **A real, partial county_code-wiring pass against app.py's highest-traffic routes** — `property_detail()` (the parcel detail page, and the original incident's own site), the shared search-resolution functions (`resolve_prop_id_to_geo_id()`, `resolve_exact_parcel()`, `search_parcels_by_address()`), and `/snapshot` (`_compute_snapshot_data()`, including closing a gap that function's own docstring had explicitly disclosed as deferred since `PARTITION-2-IMPLEMENT`). Full 218+-call-site coverage was explicitly not attempted in one pass, per the brief's own instruction preferring honest partial progress — the remaining scope is an itemized backlog, not a silent gap.
6. **One additional, previously-undocumented gap found and flagged, not fixed:** `parcel_2026_preliminary_snapshot` has no `county_code` column at all — missing from `migrate_county_partitioning.py`'s own `TABLE_SPECS` list (§1's original table inventory did not catch it either). Left unscoped in `app.py` (adding a `county_code` filter against a column that doesn't exist would break at runtime) and recorded here so it isn't silently rediscovered later — this table needs its own future migration, same `ADD COLUMN` + backfill pattern as every Mode 1 table in §5, before Dallas data can safely populate it.

### 10.6 Post-swap battery checklist — formalized for the next migration

Distilled from what this incident's remediation actually required, so the next county-key-shaped migration (or the eventual Dallas-onboarding migration itself, if `parcel_2026_preliminary_snapshot` needs one per 10.5 item 6) has a checklist rather than needing to rediscover this incident's lessons from scratch:

1. Run the migration (§5), confirmed via §9's mechanism-safety + data-correctness checks.
2. Add every transitional secondary index the migration's own call-site inventory identifies as needed (§10.3 step 1) — do this in the same migration, not a follow-up.
3. Run `ANALYZE` on every altered table — a changed PK/index shape without updated planner statistics can itself produce bad query plans independent of whether the right indexes exist.
4. Run `verify_index_coverage.py` (schema-source mode is enough at this stage; live mode requires a live DB connection) and resolve every CONFIRMED gap before considering the migration complete.
5. Check registry/budget assumptions that were sized against single-county data (e.g. any hardcoded row-count or duration expectation from §9.9-style measurement work) — a second county changes real data volume, not just row identity.
6. Restart the application process if any long-lived in-process state (e.g. `app.py`'s `_SNAPSHOT_CACHE`) could be holding pre-migration-shaped keys or values.
7. Only after 1-6: re-run `verify_index_coverage.py --index-source live` and confirm the Dallas hard gate's zero/zero result (§10.4) before loading the new county's real data.

### 10.7 Verification performed for this amendment

1. **Document-consistency check:** cross-checked against §1-§9; nothing here contradicts prior rulings. §10.3 extends §9.2(a)'s rule with a transitional lifecycle rather than changing it; §10.4 formalizes an implication of §9.2(a) rather than introducing a new requirement; §10.5 is a factual incident record.
2. **Every concrete claim in §10.5 was verified against real source before being written here** — the `_PREDICATE_RE` bug via direct interactive Python testing (not assumed), the two schema.sql index gaps via direct reading of the three named loader files' real query text, the `parcel_2026_preliminary_snapshot` gap via direct reading of both `schema.sql` and `migrate_county_partitioning.py`'s `TABLE_SPECS` list, and the county-slug format via a direct repo grep plus Diego's own explicit decision this session (not assumed from the brief's own example).
3. **Sandbox-vs-live disclosure:** this amendment's text is a document-consistency and factual-record exercise, completed fully in this sandbox from repo-read evidence and this session's own fixture-test results. The Dallas hard gate itself (§10.4) is explicitly **not** evaluated as passed or failed here — that requires the live `--index-source live` re-run against production, which only Diego can run. §10.6's checklist item 6 (process restart) and any registry/budget re-sizing (item 5) are likewise real operational steps for Diego to perform, not sandbox-verifiable.

Nothing in this amendment executes any schema change, index creation, or live database connection beyond what §10.5 item 3 (`schema.sql`'s two new `CREATE INDEX IF NOT EXISTS` statements) records as already applied to this repo's schema file — not yet run against production.
