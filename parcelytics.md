...
8. **Vault → operational staging → loader recovery, when staging is missing a source file.** Established as a standing pattern during the Mission 4/PX-20260910-01/-03 Travis `hs_cap_loss` repair (Diego hit a missing-staging-files blocker mid-backfill, resolved by a "Consultant ruling" to restore from the vault rather than alter the loader). The rule, generalized: (1) locate the source file in the canonical Raw Vault (per point 1 above); (2) verify its identity/hash/provenance against the vault's own record before trusting it; (3) restore/copy it to the loader's expected **operational staging** path (the transient, disposable location a loader's config actually reads from — e.g. `~/Parcelytics/data`, per `config.py`'s `PARCELYTICS_DATA_ROOT`, FILE-ARCH-2); (4) run the loader unmodified — do **not** change loader configuration, path constants, or `config.AJR_FILES`/archive-grammar entries merely because an expected staging artifact was absent, since the artifact being absent is a staging-disposability fact, not a loader-correctness fact; (5) verify the resulting load (gate re-run / spot-check / coverage re-measurement, per the county's normal post-load verification); (6) leave the canonical Vault itself untouched — it is read-only source-of-truth in this flow, never a write target. **Vault is authoritative; staging is disposable and reconstructable from it.** This is distinct from — and does not resolve — the still-open `[PM-supplied]` question of which of the two existing Travis vault directory structures is canonical versus a historical alias; that question is explicitly deferred to a future source/vault registry (§17) and this pattern does not rename, deduplicate, or declare either one canonical.
