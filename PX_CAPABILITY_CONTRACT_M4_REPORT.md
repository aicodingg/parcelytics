...
`python3 loaders/test_field_coverage_gate.py` — 19/19 PASS, covering all 12 D9-required cases (attribute denominator, class-conditional denominator, sparse-by-nature structural gate, sanity-floor pass, sanity-floor fail, unavailable field, unknown field, partial coverage, available coverage, zero-population edge case, missing source-column case, Travis/Dallas regression) plus three tests added across the two post-M4 review rounds: `test_tax_delinquent_low_but_nonzero_rate_still_available` and `test_tax_delinquent_zero_rows_reads_unavailable` (post-review fix, see below), and `test_tax_delinquent_measures_via_table_presence_not_column_lookup` (live-validation fix, see "Live-validation findings" below).
...
## Live-validation findings (real production run, Travis, tax_year=2023 and 2026)

Diego ran the measurement layer against real production for the first time and surfaced two issues no fixture had caught. Both are addressed or investigated below; **Mission 4 is not being marked closed until both are resolved or explicitly deferred with PM sign-off**, per the live-validation review's own instruction.

### Fix 2: `tax_delinquent`'s structural gate was unreachable dead code

**What was wrong:** `check_source_column(table, column)` looks up `information_schema.columns` for a column literally named `field_def.canonical_field` — `"tax_delinquent"` for this field. The real `tax_delinquent` table has no such column (confirmed via a live `\d tax_delinquent`); its real data columns are `delinquent_total`, `current_year_total`, `total_due`, etc. Before this fix, `measure_sparse_by_nature_field()` ran `check_source_column()` for every field, tax_delinquent included, BEFORE reaching the table's own presence-check branch. That check always returned `False` in production, so the structural gate always short-circuited to `NOT_MEASURABLE` — the presence-check code from the earlier post-review fix (documented above) was correct but **dead, unreachable code**. This was invisible to every fixture test written so far because those fixtures' fake `information_schema.columns` handler always answered "yes, the column exists," which is not what real production's catalog says for this table.

**Why it wasn't caught by fixtures:** the fixture tests were, in effect, asserting a false premise about the real schema. This is the exact failure mode live validation exists to catch — code-level unit tests can only be as honest as the assumptions baked into their fakes.

**What changed:** `measure_sparse_by_nature_field()` in `loaders/field_coverage_gate.py` now runs the `table == "tax_delinquent"` branch **first**, before and instead of `check_source_column()`, since tax_delinquent is a table-level presence question ("does this table have any rows for this county"), not a single-column question the generic gate was built for. `check_source_column()` is untouched and still correct for `exemption_codes`/`hs_cap_loss`, which really are single named columns on `parcel_tax_year`. `capability_registry.py`'s `tax_delinquent` docstring is unchanged (it already described the boolean-shaped presence logic correctly — the bug was purely in call ordering, not in what the presence check itself computed).

**Regression tests added (loud-fail, not silent-pass, fixtures):** the two existing tax_delinquent tests were tightened so their `information_schema.columns` branch now `raise`s instead of returning `(1,)` — if `check_source_column()` is ever called for tax_delinquent again, the test fails immediately rather than silently masking the bug the way the old fixture did. A third test, `test_tax_delinquent_measures_via_table_presence_not_column_lookup`, proves the field measures `MEASURED`/`Available` purely from the table-presence query, with `information_schema.columns` never queried at all. Full suite: **19/19 PASS**. `loaders/demonstrate_capability_m4.py` (8/8) and `run_offline_checks.py` (6/6) re-run clean, no regression.

**Re-run Travis/Dallas dry-run pass:** the fixture-backed `demonstrate_capability_m4.py` proof (Travis/Dallas, all 12 registered fields) shows no change in output — it never exercised `tax_delinquent`, so this fix is invisible to that script by design, exactly as before. The real dry-run re-run Diego should perform, now that this fix is in place: `python3 loaders/field_coverage_gate.py --county TRAVIS --tax-year 2023` (and the Dallas equivalent) should now report `tax_delinquent` as `MEASURED`/`Available` for Travis (assuming any delinquent rows exist) instead of the pre-fix `NOT_MEASURABLE`/`Unknown` — this is the concrete, observable before/after Diego's own live run will show.

### Fix 3 (investigation only, no code change): `hs_cap_loss` measures 0.00% live for BOTH 2023 and 2026

**The contradiction:** live production shows `hs_cap_loss` at exactly 0.00% for Travis tax_year=2023 (0 non-null of 449,493 rows) AND tax_year=2026. `data_coverage.py`'s `HS_CAP_LOSS_COVERAGE[2023] = 0.999` says otherwise — a real, sizeable, direct contradiction on a year that isn't supposed to be the "structurally always-false" 2025/2026 zone.

**Investigation findings, with evidence:**

1. **This is not a bug in `field_coverage_gate.py`'s SQL.** The query (`COUNT(*) FILTER (WHERE NULLIF(TRIM(hs_cap_loss::text), '') IS NOT NULL) FROM parcel_tax_year WHERE county_code = %s AND tax_year = %s`) is a correct, literal read of `parcel_tax_year.hs_cap_loss` — it reports exactly what's in that table for that county/year, nothing more.

2. **`data_coverage.py`'s manifest predates a real architecture change that moved where hs_cap_loss actually gets written.** `data_coverage.py` was committed 2026-07-17 (`0fd2fdf`, the "Homestead-Cap Data Integrity: Full Fix Set" brief) — its own docstring states the numbers are "SEEDED directly from confirmed live-DB queries Diego ran himself" at that time. Twelve days later, 2026-07-29 (`6d22986`, "Add unit-model architecture for multi-unit properties"), the AJR loader (`loaders/load_ajr.py`) was refactored to write `hs_cap_loss` into the **new** `prop_unit_tax_year` table (via `ears_format.PROP_UNIT_TAX_YEAR_UPSERT_SQL`) instead of writing `parcel_tax_year` directly. From that point on, `parcel_tax_year.hs_cap_loss` — the table `capability_registry.py`'s `hs_cap_loss` field definition points at, and the table this measurement layer reads — is populated **only** as a derived rollup: `parcel_rollup.py`'s `ROLLUP_SQL` computes `SUM(y.hs_cap_loss)` from `prop_unit_tax_year` and writes it into `parcel_tax_year` via `ON CONFLICT ... DO UPDATE SET hs_cap_loss = EXCLUDED.hs_cap_loss` (unconditional overwrite, not a `COALESCE`-guarded one).

3. **`parcel_rollup.py`'s own code comment (added in PARCEL-ROLLUP-HOTFIX-1, 2026-08-21, `f72532f`) discloses that this rollup step has not reliably run in production**: *"this module hasn't actually run to completion since the [county partitioning] migration finished... a loaded gun, not yet fired."* Because the rollup's `ON CONFLICT` clause unconditionally overwrites `hs_cap_loss` with whatever `SUM()` it computes — never `COALESCE`d against the prior value — any rollup pass that ran against an incomplete or not-yet-repopulated `prop_unit_tax_year` for a given county/year would silently zero out a previously-correct `parcel_tax_year.hs_cap_loss`, rather than leaving it alone.

4. **Working hypothesis, not yet confirmed live:** `data_coverage.py`'s 99.9% figure for 2023 was real and accurate *when measured*, against the pre-unit-model write path. The Unit Model refactor (D2.1 finding) then moved the authoritative hs_cap_loss data to `prop_unit_tax_year`, and the derived copy in `parcel_tax_year` (what this measurement layer and `data_coverage.py`'s original manifest both nominally describe) has since gone stale or been zeroed by an incomplete rollup pass — consistent with finding (a) (stale manifest) **and** finding (b) (the value was cleared, not "never populated") **and**, partially, finding (c) (the manifest's original measurement and this measurement layer's current read may no longer be describing the same underlying data path, even though both nominally reference "parcel_tax_year.hs_cap_loss").

**Not fixed or reconciled in this pass, per the live-validation review's explicit instruction.** This looks like it needs its own brief — likely: (a) a live check of `SELECT COUNT(*) FILTER (WHERE hs_cap_loss IS NOT NULL) FROM prop_unit_tax_year WHERE county_code='TRAVIS' AND tax_year=2023` to confirm the true, unit-grain source data is still intact (this is the single fastest way to confirm or rule out the hypothesis above); (b) if confirmed, a decision on whether `capability_registry.py`'s `hs_cap_loss` field should measure `prop_unit_tax_year` directly instead of `parcel_tax_year`, or whether `parcel_rollup.py` should be re-run in production to repopulate `parcel_tax_year` correctly (or both); (c) a decision on whether `data_coverage.py`'s manifest itself needs a refresh note or supersession marker now that it may be describing a pre-refactor state. None of these decisions were made here — this section is investigation-only, as instructed.
...
**Live-validation update:** Fix 2 (`tax_delinquent`'s structural-gate ordering bug) is resolved and verified (19/19 tests, 8/8 D6 proof points, 6/6 offline checks, all green). Fix 3 (`hs_cap_loss` 0.00% vs. `data_coverage.py`'s 99.9% for 2023) is investigated but **not resolved** — see "Live-validation findings" above. Per the live-validation review's explicit instruction, **Mission 4 remains open, not closed**, until Fix 3 is either resolved or explicitly deferred with PM sign-off as its own brief.
